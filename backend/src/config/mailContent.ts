...
export const mailContent = {
  registerUser: (verificationToken: string): IEmailContent => {
    // TODO: ユーザー登録用リンクを設定
    const link = `${process.env.SERVER_URL}/register/${verificationToken}`;
  ...
  changeEmail: (verificationToken: string): IEmailContent => {
    ...
    return {
      subject: 'メールアドレス変更',
      text: `以下のリンクをクリックしてメールアドレス変更を完了してください: ${link}`,
      html: `<p>以下のリンクをクリックしてメールアドレス変更を完了してください:</p><a href="${link}">確認リンク</a>`,
    };
  },
  forgotPassword: (verificationToken: string): IEmailContent => {
    // TODO: パスワード再設定用リンクを設定
    const link = `${process.env.SERVER_URL}/reconfig-password/${verificationToken}`;
